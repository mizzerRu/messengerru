...
import com.rumessanger.msg.ChatActivity;
...
public class ChatsAdapter extends RecyclerView.Adapter<ChatViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ChatViewHolder holder, int position) {
        ...
        String userId;
        if (!chats.get(position).getUsersId1().equals(Objects.requireNonNull(FirebaseAuth.getInstance().getCurrentUser()).getUid())) {
            userId = chats.get(position).getUsersId1();
        } else {
            userId = chats.get(position).getUsersId2();
        }
